...
func TestGRIDFxChainAppExport(t *testing.T) {
	db := dbm.NewMemDB()
	app := NewGRIDFxChainApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, 0)
	...
	app2 := NewGRIDFxChainApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, 0)
