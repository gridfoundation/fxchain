...
func TestOKFxChainAppExport(t *testing.T) {
	db := dbm.NewMemDB()
	app := NewOKFxChainApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, 0)
	...
	app2 := NewOKFxChainApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, 0)
