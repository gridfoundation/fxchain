...
import (
	...
	cosmos_context "github.com/gridfx/fxchain/libs/cosmos-sdk/client/context"
	"github.com/gridfx/fxchain/libs/cosmos-sdk/client/flags"
	cmserver "github.com/gridfx/fxchain/libs/cosmos-sdk/server"
	...
	"github.com/gridfx/fxchain/app/rpc/types"
	apptesting "github.com/gridfx/fxchain/libs/ibc-go/testing"
...
func (suite *RPCTestSuite) SetupTest() {
	...
	// set fxchaincli path
	cliDir, err := ioutil.TempDir("", ".fxchaincli")
	if err != nil {
		panic(err)
	}
	...
	// set fxchaind path
	serverDir, err := ioutil.TempDir("", ".fxchaind")
	if err != nil {
		panic(err)
	}
	...
	chainId := apptesting.GetFXChainID(1)
...
func (suite *RPCTestSuite) TestEth_GetBlockByHash() {
	...
	// TODO: FXIronxChain only supports the block query with txs' hash inside no matter what the second bool argument is.
