...
func (suite *AnteTestSuite) SetupTest() {
	...
	err := fxchain.SetChainId(chainId)
	suite.Nil(err)
...
func newTestCoins() sdk.Coins {
	return sdk.NewCoins(fxchain.NewPhotonCoinInt64(500000000))
}

func newTestStdFee() auth.StdFee {
	return auth.NewStdFee(220000, sdk.NewCoins(fxchain.NewPhotonCoinInt64(150)))
}
...
func newTestEthTx(ctx sdk.Context, msg *evmtypes.MsgEthereumTx, priv tmcrypto.PrivKey) (sdk.Tx, error) {
	chainIDEpoch, err := fxchain.ParseChainID(ctx.ChainID())
	if err != nil {
		return nil, err
	}
