...
if [ -d ${FXCHAIN_NET_CACHE}/node0/fxchaind ]; then
    seed_addr=$(${BIN_NAME} tendermint show-node-id --home ${FXCHAIN_NET_CACHE}/node0/fxchaind)@${IP}:${seedp2pport}
fi
...
init() {
    ...
    if [ -d ${FXCHAIN_NET_CACHE}/${NAME} ]; then
        echo "Invalid index!"
        echo "<${FXCHAIN_NET_CACHE}/${NAME}> already exists. Use '-n' to try another index."
    ...
    ${BIN_NAME} init ${NAME} -o --chain-id ${CHAIN_ID} --home ${FXCHAIN_NET_CACHE}/${NAME}/fxchaind --node-index ${INPUT_INDEX}
}
...
start() {
    ...
    rm ${FXCHAIN_NET_CACHE}/${NAME}/fxchaind/config/genesis.json
    cp ${FXCHAIN_NET_CACHE}/node0/fxchaind/config/genesis.json ${FXCHAIN_NET_CACHE}/${NAME}/fxchaind/config/
...
#     echo "${BIN_NAME} --home ${FXCHAIN_NET_CACHE}/${NAME}/fxchaind  start --p2p.laddr tcp://${IP}:${p2pport} --p2p.seeds ${seednode} --rpc.laddr tcp://${IP}:${rpcport}"
...
#    LOG_LEVEL=main:info,*:error,state:debug,consensus:debug
    ...
    ${BIN_NAME} start \
    --chain-id ${CHAIN_ID} \
    --home ${FXCHAIN_NET_CACHE}/${NAME}/fxchaind \
    ...
    --rpc.laddr tcp://${IP}:${rpcport} > ${FXCHAIN_NET_CACHE}/rpc${INPUT_INDEX}.log 2>&1 &
