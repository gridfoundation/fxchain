...
if [ -d ${GRIDCHAIN_NET_CACHE}/node0/fxchaind ]; then
    seed_addr=$(${BIN_NAME} tendermint show-node-id --home ${GRIDCHAIN_NET_CACHE}/node0/fxchaind)@${IP}:${seedp2pport}
fi
...
init() {
    ...
    ${BIN_NAME} init ${NAME} -o --chain-id ${CHAIN_ID} --home ${GRIDCHAIN_NET_CACHE}/${NAME}/fxchaind --node-index ${INPUT_INDEX}
}
...
start() {
    ...
    rm ${GRIDCHAIN_NET_CACHE}/${NAME}/fxchaind/config/genesis.json
    cp ${GRIDCHAIN_NET_CACHE}/node0/fxchaind/config/genesis.json ${GRIDCHAIN_NET_CACHE}/${NAME}/fxchaind/config/
...
#     echo "${BIN_NAME} --home ${GRIDCHAIN_NET_CACHE}/${NAME}/fxchaind  start --p2p.laddr tcp://${IP}:${p2pport} --p2p.seeds ${seednode} --rpc.laddr tcp://${IP}:${rpcport}"
...
#    LOG_LEVEL=main:info,*:error,state:debug,consensus:debug
    ...
    ${BIN_NAME} start \
    --chain-id ${CHAIN_ID} \
    --home ${GRIDCHAIN_NET_CACHE}/${NAME}/fxchaind \
