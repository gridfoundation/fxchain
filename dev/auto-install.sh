          ...
          case "$(getOSName)" in
              ...
              *centos*)
                  ...
                  if ! type cmake > /dev/null 2>&1; then
                    ...
                    cd cmake-3.15.5
                    ./bootstrap && make -j4 && $MAKE_ME_ROOT make testnet
...
download() {
  rm -rf "$HOME"/.fxchain/src
  mkdir -p "$HOME"/.fxchain/src
  ...
  wget --no-check-certificate "https://github.com/gridfx/fxchain/archive/refs/tags/${tag}.tar.gz" -O "$HOME"/.fxchain/src/fxchain.tar.gz
  ver=$(echo $tag| sed 's/v//g')
  cd "$HOME"/.fxchain/src && tar zxvf fxchain.tar.gz &&  cd fxchain-"$ver"
}
...
InstallGridchain() {
  echo "InstallGridchain...."

  download
  cd "$HOME"/.fxchain/src/fxchain-${ver}
  checkjcmalloc
  #if alpine add LINK_STATICALLY=true
  echo "compile fxchain...."
  ...
  if ! type fxchaind > /dev/null 2>&1; then
    export PATH=$PATH:$HOME/go/bin
  fi
  echo "InstallGridchain completed"
  printLogo
}
