...
#    fxchaind start --pruning=nothing --rpc.unsafe \
    nohup fxchaind start --rpc.unsafe \
...
cd .. && make testnet DEBUG=true Venus1Height=1 Venus2Height=1 EarthHeight=1

# Set up config for CLI
fxchaincli config chain-id $CHAINID
fxchaincli config output json
fxchaincli config indent true
fxchaincli config trust-node true
fxchaincli config keyring-backend test
...
fxchaincli keys add --recover admin16 -m "palace cube bitter light woman side pave cereal donor bronze twice work" -y
...
fxchaind add-genesis-account $(fxchaincli keys show $KEY    -a) 100000000fury --home $HOME_SERVER
fxchaind add-genesis-account $(fxchaincli keys show admin16 -a) 900000000fury --home $HOME_SERVER
fxchaind add-genesis-account $(fxchaincli keys show admin17 -a) 900000000fury --home $HOME_SERVER
fxchaind add-genesis-account $(fxchaincli keys show admin18 -a) 900000000fury --home $HOME_SERVER

# Sign genesis transaction
fxchaind gentx --name $KEY --keyring-backend test --home $HOME_SERVER
...
fxchaincli tx send captain 0x83D83497431C2D3FEab296a9fba4e5FaDD2f7eD0 1fury --fees 1fury -b block -y
