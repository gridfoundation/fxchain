...
import (
	...
	types3 "github.com/gridfx/fxchain/app/types"
	...
	types2 "github.com/gridfx/fxchain/x/evm/types"
...
func TestTxSending(t *testing.T) {
	db := dbm.NewMemDB()
	defer db.Close()
	appInfo := InitializeFXXApp(t, db, 50)
	height := int64(2)
	global.SetGlobalHeight(height - 1)
	appInfo.App.BeginBlock(abci.RequestBeginBlock{Header: abci.Header{ChainID: "fxchain-67", Height: height, Time: time.Now()}})
...
func InitializeFXXApp(b testing.TB, db dbm.DB, numAccounts int) AppInfo {
	...
	fxxApp := SetupWithGenesisAccounts(b, db, genAccs)
...
func setup(db dbm.DB, withGenesis bool, invCheckPeriod uint) (*app.FXIronxChainApp, simapp.GenesisState) {
	fxxApp := app.NewFXIronxChainApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, invCheckPeriod)
	if withGenesis {
		return fxxApp, app.NewDefaultGenesisState()
	}
	return fxxApp, simapp.GenesisState{}
}

// SetupWithGenesisAccounts initializes a new FXIronxChainApp with the provided genesis
// accounts and possible balances.
func SetupWithGenesisAccounts(b testing.TB, db dbm.DB, genAccs []authexported.GenesisAccount) *app.FXIronxChainApp {
	fxxApp, genesisState := setup(db, true, 0)
	authGenesis := authtypes.NewGenesisState(authtypes.DefaultParams(), genAccs)
	appCodec := fxxApp.Codec()
	...
	fxxApp.Commit(abci.RequestCommit{})
	fxxApp.BeginBlock(abci.RequestBeginBlock{Header: abci.Header{Height: fxxApp.LastBlockHeight() + 1}})
...
func deployOip20(info *AppInfo) error {
	...
	info.App.BeginBlock(abci.RequestBeginBlock{Header: abci.Header{ChainID: "fxchain-67", Height: height, Time: time.Now()}})
