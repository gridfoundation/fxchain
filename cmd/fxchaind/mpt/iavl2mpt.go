...
func migrateContractToMpt(migrationApp *app.FXIronxChainApp, cmCtx sdk.Context, evmMptDb ethstate.Database, evmTrie ethstate.Trie) {
	committedHeight := cmCtx.BlockHeight() - 1
	count := 0
...
func miragteBlockHashesToDb(migrationApp *app.FXIronxChainApp, cmCtx sdk.Context, batch ethdb.Batch) {
	count := 0
...
func miragteBloomsToDb(migrationApp *app.FXIronxChainApp, cmCtx sdk.Context, batch ethdb.Batch) {
	count := 0
...
func readAllParams(app *app.FXIronxChainApp) map[string][]byte {
