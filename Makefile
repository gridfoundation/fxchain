...
ifeq ($(MAKECMDGOALS),testnet)
  ldflags += -X github.com/gridfx/fxchain/libs/cosmos-sdk/server.ChainID=clockend-4200
endif
...
fxchain: check_version
	$(cgo_flags) go install -v $(BUILD_FLAGS) -tags "$(build_tags)" ./cmd/fxchaind
	$(cgo_flags) go install -v $(BUILD_FLAGS) -tags "$(build_tags)" ./cmd/fxchaincli
...
cli:
	go install -v $(BUILD_FLAGS) -tags "$(build_tags)" ./cmd/fxchaincli

server:
	go install -v $(BUILD_FLAGS) -tags "$(build_tags)" ./cmd/fxchaind
...
build:
ifeq ($(OS),Windows_NT)
	go build $(BUILD_FLAGS) -tags "$(build_tags)" -o build/fxchaind.exe ./cmd/fxchaind
	go build $(BUILD_FLAGS) -tags "$(build_tags)" -o build/fxchaincli.exe ./cmd/fxchaincli
else
	go build $(BUILD_FLAGS) -tags "$(build_tags)" -o build/fxchaind ./cmd/fxchaind
	go build $(BUILD_FLAGS) -tags "$(build_tags)" -o build/fxchaincli ./cmd/fxchaincli
endif
...
build-docker-fxchainnode:
	$(MAKE) -C networks/local

# Run a 4-node testnet locally
localnet-start: localnet-stop
	@if ! [ -f build/node0/fxchaind/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/fxchaind:Z fxchain/node testnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test ; fi
	docker-compose up -d
