...
func GetCmdDestroyValidator(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.NoArgs,
		Short: "deregister the validator from the FxChain and unbond the min self delegation",
		Long: strings.TrimSpace(
			fmt.Sprintf(`Deregister the validator from the FxChain and unbond the min self delegation.
...
Example:
$ %s tx staking proxy bind did:fury:ex1cftp8q8g4aa65nw9s5trwexe77d9t6crp8cm05 --from mykey
`,
				version.ClientName),
		),
		RunE: func(cmd *cobra.Command, args []string) error {
