...
func (suite *EvmTestSuite) TestInit() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			db := dbm.NewMemDB()
			chain := app.NewGRIDIronxChainApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, 0)
...
func (suite *EvmTestSuite) TestExport_db() {
	viper.SetEnvPrefix("GRIDIRONXCHAIN")
	...
	os.Setenv("GRIDIRONXCHAIN_EVM_IMPORT_MODE", "default")
	evm.InitGenesis(suite.ctx, *suite.app.EvmKeeper, &suite.app.AccountKeeper, initGenesis)

	tmpPath := "./test_tmp_db"
	os.Setenv("GRIDIRONXCHAIN_EVM_EXPORT_MODE", "db")
	os.Setenv("GRIDIRONXCHAIN_EVM_EXPORT_PATH", tmpPath)

	defer func() {
		os.Setenv("GRIDIRONXCHAIN_EVM_IMPORT_MODE", "default")
		os.Setenv("GRIDIRONXCHAIN_EVM_EXPORT_MODE", "default")
...
func testImport_db(suite *EvmTestSuite,
	...
	viper.Set(sdk.FlagDBBackend, string(dbm.GoLevelDBBackend))
	os.Setenv("GRIDIRONXCHAIN_EVM_IMPORT_MODE", "db")
	os.Setenv("GRIDIRONXCHAIN_EVM_IMPORT_PATH", dbPath)
...
func (suite *EvmTestSuite) TestExport_files() {
	...
	tmpPath := "./test_tmp_db"
	os.Setenv("GRIDIRONXCHAIN_EVM_EXPORT_MODE", "files")
	os.Setenv("GRIDIRONXCHAIN_EVM_EXPORT_PATH", tmpPath)
...
func testImport_files(suite *EvmTestSuite,
	...
	os.Setenv("GRIDIRONXCHAIN_EVM_IMPORT_MODE", "files")
	os.Setenv("GRIDIRONXCHAIN_EVM_IMPORT_PATH", filePath)
