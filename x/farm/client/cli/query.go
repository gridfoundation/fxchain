...
Example:
$ %s query farm rewards pool-eth-xxb did:fury:ex1cftp8q8g4aa65nw9s5trwexe77d9t6crp8cm05
`,
				version.ClientName,
			),
		),
...
func GetCmdQueryWhitelist(storeName string, cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "whitelist",
		Short: "query the whitelist of pools to farm fury",
		Long: strings.TrimSpace(
			fmt.Sprintf(`Query the current whitelist of pools which are approved to farm fury.
...
Example:
$ %s query farm account did:fury:ex1cftp8q8g4aa65nw9s5trwexe77d9t6crp8cm05
`,
				version.ClientName,
			),
		),
...
Example:
$ %s query farm lock-info pool-eth-xxb did:fury:ex1cftp8q8g4aa65nw9s5trwexe77d9t6crp8cm05 
`,
				version.ClientName,
			),
		),
