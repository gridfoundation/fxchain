...
func TestNewTokenMsgSend(t *testing.T) {
	...
	Errorcoins := sdk.SysCoins{
		sdk.NewDecCoinFromDec("fxc", sdk.NewDec(100)),
		sdk.NewDecCoinFromDec("fxc", sdk.NewDec(100)),
	...
	testCase := []struct {
		...
		{NewMsgTokenSend(fromAddr, toAddr, Errorcoins), ErrInvalidCoins("100.000000000000000000fxc,100.000000000000000000fxc,100.000000000000000000oke")},
...
func TestNewTokenMultiSend(t *testing.T) {
	...
	// correct message
	coinStr := `[{"to":"did:fury:ex1jedas2n0pq2c68pelztgel8ht8pz50rhcyttfu","amount":"1` + common.NativeToken + `"}]`
	transfers, err := StrToTransfers(coinStr)
	require.Nil(t, err)

	// coins not positive
	toAddr0, err := sdk.AccAddressFromBech32("did:fury:ex1jedas2n0pq2c68pelztgel8ht8pz50rhcyttfu")
	require.Nil(t, err)
