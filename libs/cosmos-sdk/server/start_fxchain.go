...
func InstallHoGRIDFX(flag string, hooker fnHookstartInProcess) {
...
func StopCmd(ctx *Context) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				errStr := fmt.Sprintf("%s Please finish the process of fxchaind through kill -2 pid to stop gracefully", err.Error())
				cmn.Exit(errStr)
			}
			defer f.Close()
...
func RegisterServerFlags(cmd *cobra.Command) *cobra.Command {
	...
	cmd.Flags().String(FlagUlockKeyHome, os.ExpandEnv("$HOME/.fxchaincli"), "The keybase home path")
	cmd.Flags().String(FlagRestPathPrefix, "fxchain", "Path prefix for registering rest api route.")
...
func nodeModeCmd(ctx *Context) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "node-mode",
		Short: "fxchaind start --node-mode help info",
		Long: `There are three node modes that can be set when the fxchaind start
