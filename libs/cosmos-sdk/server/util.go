...
import (
	...
	"github.com/gridfx/fxchain/libs/cosmos-sdk/server/config"
	...
	tmflags "github.com/gridfx/fxchain/libs/tendermint/libs/cli/flags"
...
func interceptLoadConfig() (conf *cfg.Config, err error) {
	...
	appConfigFilePath := filepath.Join(rootDir, "config/fxchaind.toml")
	if _, err := os.Stat(appConfigFilePath); os.IsNotExist(err) {
		appConf, _ := config.ParseConfig()
		config.WriteConfigFile(appConfigFilePath, appConf)
	}

	viper.SetConfigName("fxchaind")
